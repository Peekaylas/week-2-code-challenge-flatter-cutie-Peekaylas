fetch("https://json-server-vercel-sepia-five.vercel.app/characters/")
...
function displayCharacterDetails(id) {
  fetch("https://json-server-vercel-sepia-five.vercel.app/characters/" + id)
    .then(function (response) {
      return response.json();
    })
...
voteForm.onsubmit = function (event) {
  ...
  fetch(
    "https://json-server-vercel-sepia-five.vercel.app/characters/" + currentId,
    {
      method: "PATCH",
      headers: {
      ...
      body: JSON.stringify({
        votes:
          votesToAdd +
          parseInt(document.getElementById("vote-count").textContent),
      }),
    }
  )
    .then(function (response) {
